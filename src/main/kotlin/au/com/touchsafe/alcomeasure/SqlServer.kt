...
object SqlServer {
	...
	suspend fun storeResult(connection: io.r2dbc.mssql.MssqlConnection, user: User, result: Double) {
		try {
			val photo1Id = downloadAndStorePhoto(connection) ?: Int::class.java
			val photo2Id = downloadAndStorePhoto(connection) ?: Int::class.java
			val photo3Id = downloadAndStorePhoto(connection) ?: Int::class.java
			...
			LOGGER.debug("Stored Result:$user:$result:")
		} catch (ex: Throwable) {
	...
	private suspend fun downloadAndStorePhoto(connection: io.r2dbc.mssql.MssqlConnection): Int? {
		...
		// TODO: Insert the photo into the `Files` table.
		...
		return result.map { row, _ -> row["id", Integer::class.java]!!.toInt() }.awaitFirstOrNull()
	}
